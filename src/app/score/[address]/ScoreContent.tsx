...
export default function ScoreContent({ params, searchParams }: Props) {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center">
        ...
        <div className="text-center space-y-6">
          <div className="w-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto"></div>
          <p
  ...
  const getActivityLevel = () => {
    ...
    if (txCount > 500) return 'Very High';
  ...
  const getReputationTier = () => {
    ...
    if (scoreNum >= 750) return 'Diamond';
    if (scoreNum >= 650) return 'Gold';
    if (scoreNum >= 550) return 'Silver';
    return 'Bronze';
  };
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-slate-100 flex flex-col">
      ...
      <main className="flex-grow">
        <div className="relative w-full h-[280px] overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-primary via-blue-600 to-orange-500 opacity-40 mix-blend-screen"></div>
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(10,10,15,0)_0%,rgba(10,10,15,1)_100%)]"></div>
        ...
        <div className="max-w-4xl mx-auto -mt-32 relative z-10 px-4 pb-12">
          <div className="flex flex-col items-center">
            <div className="relative size-64 md:size-80 flex items-center justify-center rounded-full bg-[#0a0a0f] border-4 border-primary/40 neon-glow">
              <div className="absolute inset-2 border-2 border-dashed border-primary/20 rounded-full"></div>
              <div className="text-center">
                <h1 className="text-7xl md:text-8xl font-bold tracking-tighter text-slate-100">{score}</h1>
                <p className="text-primary font-bold uppercase tracking-[0.2em] text-sm mt-2">{tier}</p>
              </div>
          ...
          {metrics && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-16 mb-12">
              <MetricCard icon="⚡" label="Activity" value={getActivityLevel()} />
              <MetricCard icon="📅" label="Account Age" value={`${metrics.walletAgeDays || 0} Days`} />
              <MetricCard icon="💰" label="Total Volume" value={`$${(parseFloat(metrics.strkBalance || '0') / 1e18 * 0.5 + parseFloat(metrics.usdcBalance || '0') / 1e6).toFixed(1)}k`} />
              <MetricCard icon="🏦" label="Protocols" value={`${metrics.uniqueTokens || 0} Active`} />
              <MetricCard icon="🕐" label="Tx Count" value={metrics.txCount?.toLocaleString() || '0'} />
              <MetricCard icon="✅" label="Reputation" value={`${getReputationTier()} Tier`} />
            </div>
          )}
      ...
      <footer className="bg-[#0a0a0f] border-t border-primary/10 py-12 px-6 lg:px-20">
        ...
        <div className="max-w-6xl mx-auto mt-12 pt-8 border-t border-primary/5 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-slate-600 text-xs">© 2024 Starknet Credit Score. Built on Starknet.</p>
